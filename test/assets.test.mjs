...
/**
 * ★プロンプトの中で同じものを要求しつつ禁止していないか。
 *
 * 抜き板のレシピは「床を描くな」と言う（板の下に床が焼き込まれると必ず浮く）。
 * ところが対象の説明に「床に置かれた〜」と書いてあると、完成プロンプトが
 * 自分と矛盾する。実際に `coat_discarded` が
 * 「discarded in a heap on the floor」＋「no floor」になっていた。
 * 矛盾したプロンプトは判断を人に押し戻すので、機械的に落とす。
 */
test('★完成プロンプトが自分と矛盾していない（床を禁じつつ床を要求しない）', () => {
  for (const a of ASSETS) {
    const r = RECIPES[a.r];
    if (!/no ground plane|no floor/i.test(r.tech)) continue;
    assert.ok(!/\bfloor\b|\bground\b/i.test(a.s),
      `${a.p}: レシピが床を禁じているのに対象の説明が床に触れている（「${a.s}」）`);
  }
});

...
/**
 * ★ページの区切りは「行数」ではなく**実測の高さ**で決める。
 *
 * 行数で数えていたので、長い行が折り返したぶんを勘定できず、
 * 1ページ目からスクロールが必要になっていた
 * （「看板。「どなたもどうかお入りください。決してご遠慮はありません」」は
 *   1行として数えていたが、画面では2行になる）。
 *
 * 枠の高さ・字の大きさ・行間・画面幅のどれが変わっても、
 * 高さで測っていれば追従する。定数で調整する必要がなくなる。
 */
const PAGE_SLACK = 2;   // 1px の誤差でページが増えないための余裕
...
export function createUI(root, hooks = {}, opt = {}) {
  ...
  /**
   * 操作の段。★枠の外に置く。枠のすぐ上に、独立した段として積む。
   *
   * 以前は所持品と選択肢を本文と同じ固定の高さの箱に入れていた。
   * 箱の高さは変わらないのに中身の行数は画面の大きさで変わるので、
   * 画面が小さいと必ず誰かが押し出された。実機（iPhone 12 mini・667x375）では
   * 本文2行目の下半分が切られてボタンと重なり、重い場面ではボタンが枠の外へ出た。
   *
   * 場所を取り合わせない。操作は枠の外で必要な行数だけ上へ伸び、
   * 枠は本文だけを持って高さを固定できる。これで競合が起きる余地がなくなる。
   */
  ...
  function applySize() {
    ...
    /**
     * ★操作の段は枠の外にあるので、枠の状態を親にも書く。
     * CSS は後ろの兄弟から前の兄弟を選べない（:has を使わずに済ませる）。
     */
    root.dataset.size = panel.dataset.size;
   ...
   */
  /**
   * @param {boolean} dropBody 本文を作らない（据え置きの本文を持ち込まない）
   */
  function buildText(v, sameBody, dropBody) {
    textArea.innerHTML = '';
    ...
    /**
     * ★据え置きの本文は、新しいログがあるなら持ち込まない。
     *
     * 残すと枠（3行）に入りきらず、上の行が半分に切れる（実際に切れていた）。
     * 看板の文面は壁の板に描いてあるし、「看板の前に立つ」でもう一度出るので、
     * ここで消えても読み返せなくなる訳ではない。
     * ログが無いとき（小物の入れ替えなど）は、本文しか出すものがないので残す。
     */
    const body = dropBody ? null : make('body', v.body);
    ...
    boxes = order;
    return { log: log ? log.paras : [], body: body ? body.paras : [] };
  }

  /**
   * スクロールなしで読める量に切る。
   *
   * 測るために一度すべて表示して高さを取り、そのあと本文は typer が消してから送る。
   * ★段落の「送り」は offsetTop の差で取る。offsetHeight では余白が入らない。
   */
  function paginate(paras) {
    if (!paras.length) return [[]];
    for (const p of paras) { p.el.style.display = ''; p.el.textContent = p.text; }

    const tops = paras.map((p) => p.el.offsetTop);
    const bottom = tops.length
      ? tops[tops.length - 1] + paras[paras.length - 1].el.offsetHeight
      : 0;
    const advance = (i) => (i + 1 < tops.length ? tops[i + 1] - tops[i] : bottom - tops[i]);
    const limit = Math.max(24, textArea.clientHeight - PAGE_SLACK);

    const out = [];
    let cur = [];
    let h = 0;
    paras.forEach((p, i) => {
      const add = Math.max(1, advance(i));
      if (cur.length && h + add > limit) { out.push(cur); cur = []; h = 0; }
      // ページ先頭の空行は捨てる（頭が空くと1行ぶん損する）
      if (!cur.length && p.text === '') { p.el.style.display = 'none'; return; }
      cur.push(p);
      h += add;
    });
   ...
   * 区切り線は「見えている2つ目以降」にだけ付ける。
   */
  function paintBoxes() {
    let shown = 0;
    for (const b of boxes) {
      const has = b.paras.some((p) => p.el.style.display !== 'none');
      b.box.style.display = has ? '' : 'none';
      b.box.classList.toggle('sep', has && shown > 0);
      if (has) shown += 1;
    }
  }
  ...
  function render() {
    ...
    const dropBody = sameBody && v.log.length > 0;
    const text = buildText(v, sameBody, dropBody);

    head.textContent = v.head ?? '';
    head.classList.toggle('shown', !!v.head);

    /**
     * ★ページを区切る前に、選択肢と枠の大きさを確定させる。
     * 選択肢が2段に折り返すと本文の領域が縮むので、
     * 先に区切ると入りきらないページができる。
     */
    buildChoices(v);
    applySize();
    ...
    // 据え置きの本文を残す場合（ログが無いとき）は、送らずにそのまま出す
    if (sameBody && !dropBody) for (const p of text.body) p.el.textContent = p.text;
     ...
     */
    const lines = [v.head ?? '', ...v.log, ...(sameBody ? [] : v.body)];
    ...
    const begin = () => {
      ...
      /**
       * ★hold でログを読ませたなら、そのログは新しい場面に持ち込まない。
       *
       * 前の場面で読み終えたものを残すと、枠（3行）に入りきらず上へ流れて、
       * いま読む文の頭が切れる（実際に切れていた）。
       * ページの一覧から外す——`showPage` は「i 以下のページを全部出す」ので、
       * 表示だけ消しても次の `showPage` で戻ってしまう。
       */
      if (holdUntil >= 0) {
        for (const pg of pages.slice(0, holdUntil + 1)) {
          for (const q of pg) q.el.style.display = 'none';
        }
        pages = pages.slice(holdUntil + 1);
        holdUntil = -1;
      }
  ...
  return {
    ...
    skipping: () => atOnce,
    /** 自動検証用。場面が変わる前のログ待ちの状態 */
    holdInfo: () => ({ phase, holdUntil, logCount, pages: pages.length }),
    setPanelSize(size) {

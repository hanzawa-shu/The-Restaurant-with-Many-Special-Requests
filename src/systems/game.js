...
export function view(s) {
  ...
  switch (s.step) {
    ...
    case 'r_pickup': {
      ...
      body.push(`——${R.name}（逆走）`, '');
      if (p.kind === 'gun') {
        body.push('床に、自分の鉄砲が転がっている。埃をかぶっている。');
      } else {
        body.push(
          '脱ぎ捨てた外套と靴が、そのまま積まれている。',
          '内ポケットに、店で出された前菜が残っている。'
        );
      }
      choices.push({
    ...
    case 'r_fire':
      body.push('鉄砲を構えた。背後の暗がりで、何かが動いている。');
...
function returnRoomFlavor(s, n) {
  switch (n) {
    case 6: return '皿が積まれている。白い布に、乾いていない染みが広がっている。';
    case 5: return 'クリームの壺が倒れ、床に白い池をつくっている。';
    case 4: return '衣裳掛けが横倒しになっている。鏡に長い割れ目が入っている。';
    case 3: return '空の銃架。弾薬箱の蓋が開いている。';
    case 2: return '磨かれた床。姿見に、下着姿の自分が映っている。';
    case 1: return '玄関。明かりはすべて消えている。';
    default: return '';
  }
}
  ...
  switch (s.step) {
    ...
    case 'm2':
      if (id === 'bury') {
        ...
        s.dogBuried = true;
        log(s, '土を掘り、二疋を並べて埋めた。首輪の笛だけ手に残った。', MOUNTAIN_REACTION.bury);
        s.spoke = true;
      } else if (id === 'take') {
        addItem(s, 'dog_whistle');
    ...
    case 'o_reward': {
      ...
      if (id === 'reward_partner' && !s.partnerBroken) {
        ...
        log(s, '相方に渡した。相方は黙って受け取り、少し顔色が戻った。');
      } else {
        grantReward(s, amount, 'self');
        log(s, '自分が食べた。腹の底が少しだけ暖かくなった。');
      }
      return advanceOutbound(s);
    }
    ...
    case 'reveal': {
      if (id === 'examine_jar') {
        ...
        log(s, '立派な青い瀬戸の塩壺だ。中身は塩。ただし、ずいぶん粗い。',
               '菜っ葉の匂いがする。塩でよくもんだ、菜っ葉の匂いだ。');
        return s;
      }
      if (id === 'salt') {
        s.saltApplied = true;
    ...
    case 'r_pickup': {
      ...
      if (id === 'take') {
        ...
        s.hasCoat = true;
        eatOrWarm(s, 3);
        log(s, '外套を着て、靴を履いた。体の震えが止まった。そして重い。');
      } else {
        log(s, '手を出さなかった。');
      }
      s.step = nextReturnStep(s);

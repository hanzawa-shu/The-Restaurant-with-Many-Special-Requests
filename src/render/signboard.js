...
function findPanel(c, w, h) {
  ...
  const run = (n, m, at) => {
    // 「その行（列）の半分以上が明るい」区間のうち最長を返す
    let best = null;
    let cur = null;
    for (let a = 0; a < n; a++) {
      let bright = 0;
      for (let b = 0; b < m; b++) if (at(a, b) >= th) bright++;
      if (bright * 2 >= m) {
        cur = cur ?? { a, b: a };
        cur.b = a;
      } else {
  ...
  const vr = run(rows, cols, (j, i) => lum[j * cols + i]);
  const hr = run(cols, rows, (i, j) => lum[j * cols + i]);
  // 小さすぎる当たりは誤検出。板の 3割に届かないものは使わない
  if (!vr || !hr || (vr.b - vr.a) < rows * 0.3 || (hr.b - hr.a) < cols * 0.3) return fallback;

...
export async function buildRoom(scene, roomNumber, direction) {
  ...
  await Promise.all(list.map(async (p) => {
    ...
    if (g !== group) return;
    const mesh = makeBillboard(tex, p.w, p.h, { x: p.x, z: p.z, ...pose(p) });
    mesh.name = path;
 ...
 * 板の姿勢を配置データから決める。
 *
 * ★`flat: true` は「床と同じ水平面に寝かせる」意味で、傾き角を自由に渡す口は開けない。
 * 中途半端に倒した板（立面図の絵を少しだけ寝かせる）は、絵の遠近と場面の遠近が
 * 噛み合わないまま角度だけ付くので、垂直のままより悪くなる（実際に悪くなった）。
 * 低い家具は絵の側を見下ろし視で描き直す（assets.js の prop_low）のが正解で、
 * その絵は垂直の板に貼る。だから傾きが要るのは「本当に寝ているもの」だけ。
 *
 * 寝かせる板は影を落とさない。床と同一面なので、影が絵の真横にずれて二重に見える。
 * y をわずかに持ち上げるのは床との z-fighting を避けるため。
 */
const FLAT_LIFT = 0.012;

function pose(p) {
  if (p.flat) {
    return {
      y: FLAT_LIFT, azimuth: p.ry ?? AZIMUTH, tilt: Math.PI / 2,
      shadow: false, receive: true,
    };
  }
  return { y: p.y ?? null, azimuth: p.ry ?? AZIMUTH };
}
...
export async function setExtras(scene, list) {
  ...
  await Promise.all((list ?? []).map(async (p) => {
    ...
    const m = makeBillboard(tex, p.w, p.h, { x: p.x, z: p.z, ...pose(p) });
    m.name = p.tex;
    g.add(m);
  }));
}
